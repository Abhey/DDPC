	<?php
		...
		$query = "Select * from examinarpanel where type = 'Ph.D Thesis Evaluation Board' AND reg_no = '$student_reg_no' group by reg_no";
		$thisQuery = mysqli_query($connection, $query);
		if(mysqli_num_rows($thisQuery) != 0) {
        	echo "<script>
			alert('Ph.D Thesis Evaluation Board of this student has already been formed.');
