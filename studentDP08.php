<?php
    ...
    {
        $query = "SELECT * FROM faculty JOIN department WHERE faculty_id = '$faculty_id'";
...
<div class="wrapper">
    ...
    <div class="main-panel">
        ...
        $query ="SELECT name,e.reg_no , status, progress FROM examinarpanel e JOIN studentmaster s ON e.reg_no=s.reg_no WHERE status = 'pending' AND type = 'Ph.D Comprehensive Examination' GROUP BY reg_no";
  ...
  while( mysqli_num_rows($allStudents) !=0 && $thisStudent = mysqli_fetch_array($allStudents) )
   {
      ...
      $app_reg_no = $thisStudent['reg_no'];
       ...
       //$query = "SELECT * FROM examinarpanel JOIN faculty JOIN department WHERE reg_no = '$app_reg_no'";
        $query = "SELECT * FROM examinarpanel e JOIN faculty f JOIN department d ON e.faculty_id=f.faculty_id and f.dept_id=d.dept_id WHERE reg_no = '$app_reg_no'";
