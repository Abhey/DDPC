<?php
	...
	if(isset($_POST["submit"])) {
	    ...
	    if($check !== false) {
	        ...
	        $uploadOk = 0;
	        header("Location: ./user.php?img_type=0");
	        exit();
	    }
	}
	...
	if ($_FILES["photo"]["size"] > 500000) {
	    ...
	    $uploadOk = 0;
	    header("Location: ./user.php?img_type=1");
	    exit();
	}
	...
	&& $imageFileType != "gif" ) {
	    ...
	    $uploadOk = 0;
	    header("Location: ./user.php?img_type=2");
	    exit();
	}
	// Check if $uploadOk is set to 0 by an error
	if ($uploadOk == 0) {
	    // echo "Sorry, your file was not uploaded.";
	    header("Location: ./user.php?img_type=3");
	    exit();
	...
	// if everything is ok, try to upload file
	    ...
	    if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
	        ...
	        header("Location: ./user.php?img_type=5");
	    }
	}
?>